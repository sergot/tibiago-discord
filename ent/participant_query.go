...
func (pq *ParticipantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Participant, error) {
	...
	if query := pq.withBosslist; query != nil {
		if err := pq.loadBosslist(ctx, query, nodes, nil,
			func(n *Participant, e *Bosslist) { n.Edges.Bosslist = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ParticipantQuery) loadBosslist(ctx context.Context, query *BosslistQuery, nodes []*Participant, init func(*Participant), assign func(*Participant, *Bosslist)) error {
	...
	for i := range nodes {
		if nodes[i].bosslist_participants == nil {
			continue
		}
		fk := *nodes[i].bosslist_participants
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	query.Where(bosslist.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "bosslist_participants" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
